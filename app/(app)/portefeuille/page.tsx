...
export default function PortefeuillePage() {
  ...
  return (
    <div className="max-w-[1000px] mx-auto min-h-[80vh] space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-textPrimary tracking-tight">{t("wallet_title")}</h1>
        <p className="text-textSecondary mt-1">{t("wallet_subtitle")}</p>
      </div>
        ...
        <div className="md:col-span-2 bg-gradient-to-br from-primary to-[#22C55E] rounded-3xl p-8 text-white shadow-xl shadow-primary/20 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            <div className="flex items-center gap-2 text-white/90 font-medium mb-4">
              <Wallet size={20} /> {t("wallet_balance")}
            </div>
            
            <div className="mb-6">
              <h2 className="text-5xl md:text-6xl lg:text-7xl font-extrabold font-mono tracking-tight whitespace-nowrap">0 <span className="text-3xl md:text-4xl font-bold text-white/80">FCFA</span></h2>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button className="flex items-center gap-2 bg-black/20 hover:bg-black/30 text-white px-6 py-3 rounded-xl font-bold transition-colors backdrop-blur-sm">
                <ArrowUpRight size={18} /> {t("wallet_withdraw")}
              </button>
              <button className="flex items-center gap-2 bg-black/20 hover:bg-black/30 text-white px-6 py-3 rounded-xl font-bold transition-colors backdrop-blur-sm">
        ...
        <div className="bg-surface border border-border rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex justify-between mt-4 pt-4 border-t border-border">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-textPrimary">- 0</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden mt-8">
        <div className="flex border-b border-border">
          <button 
            className={`flex-1 py-4 text-sm font-bold transition-colors ${activeTab === 'Historique' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-textSecondary hover:bg-gray-50 dark:hover:bg-slate-800'}`}
            onClick={() => setActiveTab('Historique')}
          >
            {t("wallet_history")}
          </button>
          <button 
            className={`flex-1 py-4 text-sm font-bold transition-colors ${activeTab === 'Attente' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-textSecondary hover:bg-gray-50 dark:hover:bg-slate-800'}`}
            ...
            <div className="divide-y divide-border">
              {transactions.length === 0 ? (
                <div className="p-12 text-center text-textSecondary flex flex-col items-center">
                  ...
                  <p>{t("wallet_empty_tx")}</p>
                </div>
              ) : (
                transactions.map(tx => (
                  <div key={tx.id} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors group">
